<script>
	...
	async function handleSave(eventData) {
		...
		try {
			...
			if (editingEvent) {
				...
				events = events.map((e) =>
					e.id === eventData.id || e._id === eventData._id ? eventData : e
				);
			} else {
				// Add new event
	...
	$: pastEvents = events
		.filter((e) => getEventEndDate(e) < new Date())
		.sort((a, b) => getEventStartDate(b).getTime() - getEventStartDate(a).getTime());
</script>
		...
		{#if upcomingEvents.length > 0}
			<div class="events-grid">
				{#each upcomingEvents as event (event.id)}
					<EventCard {event} editable={true} onEdit={openEditModal} onDelete={handleDelete} />
				{/each}
			</div>
		{:else}
			<div class="empty-state theme-glass">
				...
				<button class="add-first-btn" onclick={openAddModal}> Add Your First Event </button>
			</div>
		{/if}
	</section>
...
<!-- Event Modal -->
<EventModal isOpen={isModalOpen} event={editingEvent} onSave={handleSave} onCancel={closeModal} />
